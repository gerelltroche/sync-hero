  ...
  return (
    ...
    <div className='h-96'>
      ...
      {folders.length > 0 ? <ol className='max-h-96 border border-gray-200 rounded overflow-scroll shadow-md'>
        {folders.map(folder => {
          return <li key={crypto.randomUUID()} className='px-4 py-2 bg-white hover:bg-sky-100 hover:text-sky-900 border-b last:border-none border-gray-200 transition-all duration-300 ease-in-out'>{folder}</li>
        })}
      </ol> : null}
    </div>
    </>
